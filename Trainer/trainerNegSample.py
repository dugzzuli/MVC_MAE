...
class TrainerNegSample(object):
    def __init__(self, model, config):
        self.config = config
        self.model = model
        self.loadW=config['loadW']
            ...
            if epoch % 500 == 0:
                ...
                while True:
                    ...
                    if train_emb is None:
                        ...
                        train_emb = np.concatenate((train_emb, emb),0)
                ...
                train_embH=np.nan_to_num(train_embH)
                train_emb = np.nan_to_num(train_emb)
                if(1==1):
                    acc, nmi = node_clustering(train_embH, train_label)

                    # for bbb in [2,3,4,5,6,7,8,9,10]:
                    #     node_clustering_kmean(train_embH, train_label,bbb)
